...
export async function callTool(api, name, args = {}) {
  ...
  switch (name) {
    ...
    case "install_tool": {
      const r = await api.call("POST", `/v1/tools/${encodeURIComponent(need("name"))}/install`, args.config && typeof args.config === "object" ? { config: args.config } : undefined);
